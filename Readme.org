#+title: Readme
* Introduction
=mpisee= is your friendly neighborhood MPI profiler. It can reveal communication bottlenecks and uncover hidden patterns within MPI communicators.
#+attr_html: :width 100%
[[./images/Updated.jpg]]
* Build
** Dependencies
...
- Display all data by separating ranks:
  #+begin_src bash
  python 3/path/to/mpisee-through.py -i /path/to/mpisee_profile.db -a
  #+end_src
- Display data for collective MPI operations only:
  #+begin_src bash
  python3 /path/to/mpisee-through.py -i /path/to/mpisee_profile.db -c
  #+end_src
- Display data for point-to-point MPI operations only:
  #+begin_src bash
  python3 /path/to/mpisee-through.py -i /path/to/mpisee_profile.db -p
  #+end_src
