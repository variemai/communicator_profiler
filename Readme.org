...
- ~LD_PRELOAD=/path/to/libmpisee.so MPISEE_OUTFILE=/path/to/outputfile.db <mpi launcher> <args> /path/to/exec <args>~
  - ~MPISEE_OUTFILE~ defines the output file.
    - *Always use a new file for each profile, overwriting an existing file will distort the output.*
...
- To get a summary of the results parse the output file with the ~mpisee-through.py~ script located in ~mpisee-through~ folder
  - Run ~/path/to/mpisee-through.py --help~
** Results analysis with mpisee-through
- Display all data in each communicator by summarizing across ranks: ~/path/to/mpisee-through-db.py -i /path/to/mpisee_profile.db~
- Display all data by separating ranks: ~/path/to/mpisee-through-db.py -i /path/to/mpisee_profile.d -a~
